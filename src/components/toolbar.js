...
export default class MapToolbar extends React.Component{
  ...
  setActive(active,subactive=""){
    this.setState(state => {
      if(state.active == active){
        if(subactive == "") return {active: "", subactive: ""}
        else if(state.subactive == subactive) return {active: active, subactive: ""}
        else return {active: active, subactive: subactive}
      }
      else{
  ...
  renderEdit(){
    return (
      <ToolbarGroup>
        <IconButton onClick={() => this.setActive("edit","terrain")}>
      ...
      return (
        <Paper zDepth={2} className={"terrain-view"}>
          <div style={{padding: "12pt"}}>
            ...
            <RaisedButton>Render</RaisedButton>
          </div>
        </Paper>
      )
    }else return null
